...
def tool(tool_function: Callable = None, *, output_schema: dict[str, Any] | None = None) -> Tool:
    """
    ...
    Args:
        ...
        output_schema (`Dict[str, Any]`, *optional*): Custom JSON schema defining the expected 
            structure of the tool output. If provided, this will override any automatically 
            detected schema from Pydantic models or type hints.
    """
    def _tool_decorator(func: Callable) -> Tool:
        ...
        # Set output_schema with priority: explicit parameter > auto-detected > none
        if output_schema is not None:
            # Explicit output_schema parameter takes priority
            SimpleTool.output_schema = output_schema
        elif "output_schema" in tool_json_schema:
            # Legacy support for output_schema in JSON schema
            ...
            # Auto-detected schema from Pydantic models
        ...
        if func_node.decorator_list:
            # Find tool decorators - handle both @tool and @tool(...) patterns
            tool_decorators = []
            for d in func_node.decorator_list:
                if isinstance(d, ast.Name) and d.id == "tool":
                    # @tool
                    tool_decorators.append(d)
                elif isinstance(d, ast.Call) and isinstance(d.func, ast.Name) and d.func.id == "tool":
                    # @tool(...)
                    tool_decorators.append(d)
    ...
    # Handle decorator pattern: @tool or @tool(output_schema=...)
    if tool_function is None:
        # Called as @tool(output_schema=...) - return decorator
        return _tool_decorator
    else:
        # Called as @tool - apply decorator directly
        return _tool_decorator(tool_function)
